...
public:
    enum {ADD, MUL, DIV, MOD ,SUB, AND, OR, LESS, MOREEQUAL, LESSEQUAL, EQUAL, NOEQUAL, MORE};
...
{
public:
    std::vector<ConstId*> CIdList;
    std::vector<AssignStmt*> Assigns;
    ConstIdList(std::vector<ConstId*> CIdList, std::vector<AssignStmt*> Assigns) : CIdList(CIdList), Assigns(Assigns) {};
    void output(int level);
    void typeCheck();
    void genCode();
};
...
private:
    ConstIdList *CIdList;
public:
    ConstDeclStmt(ConstIdList *CIdList) : CIdList(CIdList){};
    void output(int level);
    void typeCheck();
    void genCode();
};
...
private:
    ...
    BasicBlock* end_bb;
public:
    WhileStmt(ExprNode *cond) : cond(cond) {};
    void setloop(StmtNode *loop){this->loop=loop;};
    void output(int level);
    void typeCheck();
    void genCode();
...
public:
    ...
    Type* getvaluetype()
    {
        return this -> retValue -> getOperand() -> getType();
    }
    void typeCheck();
    void genCode();
};
